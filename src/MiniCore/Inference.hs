...
setContext decl = modify $ \s -> s { tiDecl = Just decl }

-- Make context declaration out of name and expression
setNamedExpr :: Name -> Expr -> TI ()
setNamedExpr name expr = setContext combinator
 where
  combinator = case expr of
...
fromKeys m (x:xs) = case Map.lookup x m of
...
checkDataTypes = foldM checkDataType
 where
  checkDataType env decl@(Data name vars constructors) = do
    setContext decl
    s <- withNames vars
...
checkConstructors mkScheme vars = foldM checkConstructor
 where
  checkConstructor env (Constructor name args) = do
    when (not (tvars args `Set.isSubsetOf` vars)) $
...
-- Variable
tcExpr env (Var x) = do
  scheme <- case Map.lookup x env of
...
-- Lambda
tcExpr env (Lambda args e) = do
  schemes <- toSchemes args
...
-- Case
tcExpr env (Case scrutinee alts) = do
  (s, t) <- tcExpr env scrutinee
  (s', (scrut', t')) <- tcAlts env (apply s t) alts
  s'' <- unify s' (t, apply s' scrut')
  return (s'' `scomp` s' `scomp` s, t')

-- Non-recursive Let
tcExpr env (Let False bindings expr) = do
...
-- Recursive Let
tcExpr env (Let True bindings expr) = do
  let (names, exprs) = unzip bindings
...
tcAlts env scrut alts = do
  t <- fresh
  foldM combine (idSubst, (scrut, TVar t)) alts
 where
  combine (s, (scrut, t)) alt = do
    (s', (scrut', t')) <- tcAlt env alt
...
tcAlt env (PCon name, names, body) = do
  cons <- gets tiCons
...
generalize env t = do
  let vs = tvars t `Set.difference` tvars env
...
addDecls env ns ts = do
  schemes <- mapM (generalize env) ts
...
findDups (Let recursive bindings expr) = do
  let (names, exprs) = unzip bindings
 ...
 where
  scan seen name = do
    when (name `Set.member` seen) $
      raise ("Duplicate binding for " ++ quote name)
    return (Set.insert name seen)

findDups (Lambda args e) =
  Lambda args <$> findDups e

findDups (App e1 e2) =
  App <$> findDups e1 <*> findDups e2

findDups (Case scrutinee alts) =
  Case <$> findDups scrutinee <*> mapM findDupsAlt alts
 where
  findDupsAlt (PCon name, names, body) = do
    body' <- findDups body
...
convertToExpr combinators = do
  (bindings, main) <- foldM convert ([], Nothing) combinators
...
inferTypes program = do
  -- Break program into data declarations and supercombinators
