...
instance Format Expr where
  ...
  formatPrec prec (BinOp op e1 e2) =
    case lookup op precByOp of
      Just prec' ->
        parensIf (prec' < prec) (formatPrec prec' e1 <+> text op <+> formatPrec prec' e2)
      Nothing ->
        error ("Unrecognized infix operator " ++ op)
  ...
  formatPrec prec (Lambda args body) =
    parensIf (prec > lowestPrec)
      (text "\\" <> combine args <+> text "->" <+> formatPrec lowestPrec body)

-- Format name = expression pairs
instance Format [(Name, Expr)] where
  formatPrec prec bindings =
    vcat (punctuate semi (map formatBinding bindings)) <> semi
   where
...
instance Format [Alt] where
  formatPrec prec alts =
    vcat (punctuate semi (map formatAlt alts)) <> semi
   where
    formatAlt (pattern, args, expr) =
      format pattern <+> combine args <+> text "->" <+> formatPrec prec expr

-- Format constructor, wildcard, or internal tagged pattern
instance Format Pattern where
  format (PCon constructor) =
    text constructor
...
formatDefs state = text "Definitions" <> colon $$ nest 4 defs
 where
  defs = vcat (map (formatSC state) (gmGlobals state))

-- Format a single supercombinator
formatSC :: GMState -> (Name, Addr) -> Doc
formatSC state (name, addr) = text name <> colon $$ nest 4 (formatCode code)
 where
...
formatState state =
  text "State" <+> parens (formatStats (gmStats state)) <> colon $$ nest 4 components
 where
  components =
    formatStack state $$
    formatVStack state $$
...
formatStats stats =
  text "step" <+> int (gmSteps stats) <> comma <+> text "gc" <+> int (gmCollections stats)

-- Format nodes on stack
formatStack :: GMState -> Doc
formatStack state =
  text "Stack" <> colon $$ nest 4 (vcat (map (formatNode state) (reverse (gmStack state))))

-- Format numbers in V-stack
formatVStack :: GMState -> Doc
formatVStack state =
  text "V-Stack" <> colon $$ nest 4 (vcat (map (text . show) (reverse (gmVStack state))))

-- Format first n addresses on stack 
formatShortStack :: GMStack -> Int -> Doc
formatShortStack stack n =
  text "Stack" <> colon <+> hsep (punctuate comma (shorten n (map formatAddr (reverse stack))))
...
formatCode code = text "Code" <> colon $$ nest 4 (formatInstructions code)
 where
  ...
  formatInstruction (Cond t f) =
    text "Cond" $$ nest 4 (vcat
      [ text "True"  <> colon <+> formatInstructions t
...
formatDump state = format (gmDump state)
 where
  format [] =
    empty
  ...
  format ((code, stack, vstack):_) =
    text "Dump" <> colon $$ nest 4 (formatShortStack stack 3 $$ formatShortCode code 3)

-- Format a single node
formatNode :: GMState -> Addr -> Doc
formatNode state addr = formatAddr addr <> colon <+> draw (hLoad (gmHeap state) addr)
 where
  draw (NNum n) =
    int n
  ...
  draw (NApp a1 a2) =
    text "App" <+> formatAddr a1 <+> formatAddr a2

  draw (NPointer a) =
    text "Pointer to" <+> formatAddr a
