...
freeVars = map $ \(Combinator name args body) -> (name, args, walk (Set.fromList args) body)
  where
    walk :: Set.Set Name -> Expr -> FVExpr
    ...
    walk vars (BinOp op e1 e2) = walk vars (App (App (Var op) e1) e2)
    walk vars (App e1 e2) = (vars', AApp e1' e2')
      where
        e1' = walk vars e1
        e2' = walk vars e2
        vars' = freeVarsOf e1' `Set.union` freeVarsOf e2'
    walk vars (Lambda args body) = (vars', ALambda args body')
      where
        argSet = Set.fromList args
        body' = walk (vars `Set.union` argSet) body
        vars' = freeVarsOf body' `Set.difference` argSet
    walk vars (Case expr alts) = (vars', ACase expr' alts')
      where
        expr' = walk vars expr
        alts' = map (walkAlt vars) alts
        vars' = freeVarsOf expr' `Set.union` (flattenSet $ map freeVarsOfAlt alts')
    walk vars (Let recursive defs body) = (vars', ALet recursive defs' body')
      where
    ...
    walkAlt :: Set.Set Name -> Alt -> FVAlt
    walkAlt vars (tag, args, rhs) = (tag, args, rhs')
      where
        argSet = Set.fromList args
